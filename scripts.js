...
//--------------------------------------Validación rudimentaria de campos-------------------------------------------------
function nombreValido(n){nOK=/^[a-zA-z]+(\s{1}[a-zA-z]+)*$/; if(nOK.test(n.value)){return true;} else return false} //solo letras y espacios
function apellidoValido(a){aOK=/^[a-zA-z]+(\s{1}[a-zA-z]+)*$/; if(aOK.test(a.value)){return true;} else return false} //solo letras y espacios
function eMailValido(e){mailOK=/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,4})+$/; if(mailOK.test(e.value)){return true;} else return false}
function cantidadValida(c){cantOK=/^\d+$/; if(cantOK.test(c.value)){return true;} else return false} //uno o más dígitos
//------------------------------------------------------------------------------------------------------------------------
...
function validarYMostrar(n, a, e, c){           //Si todos los campos contienen valores válidos, muestra el resumen de los
    if(nombreValido(n)){                        //datos ingresados para que el usuario los controle, y continúe o cancele la compra.
        if(apellidoValido(a)){                  //Si no, selecciona el primer campo con datos inválidos para su corrección.
            if(eMailValido(e)){
                if(cantidadValida(c)){console.log('campos validados')
                    s=(c.value>1)? "s":"";
                    r="Por favor, verifique si la información ingresada es correcta:\n\nYo, " + n.value+" "+ a.value + ", deseo comprar " + c.value +  " entrada"+ s +" de la categoría \""+categoria.value+"\" para la conferencia Bs As, por un precio total de $" +valorCalculado(c,categoria)+ ". Enviar entradas y comprobante de pago a la dirección de eMail "+ e.value; console.log('mensaje generado');
                    vendidas=confirm(r);
                    return vendidas;
